...
export default function QuestionPage({ params }: { params: { id: string } }) {
  ...
  const isFreeQuestion = currentId === 1; // 첫 질문만 무료
  ...
  const handleNext = () => {
    if (nextId) {
      router.push(`/prototype11-option1/questions/${nextId}`);
    } else {
      router.push('/prototype11-option1/dashboard');
    }
  };

  const handlePrev = () => {
    if (prevId) {
      router.push(`/prototype11-option1/questions/${prevId}`);
    }
  };
  ...
  return (
    <div className="px-6 py-8 space-y-6">
        ...
        <div className="flex items-center justify-between">
          <Link href="/prototype11-option1/dashboard" className="text-gray-500">
            ← 뒤로
          </Link>
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-sm text-gray-700">
                  답변 작성 없이 다른 개발자들의 답변을 확인할 수 있어요
                </p>
              </div>
            </div>
          </div>
        ...
        {/* Others' Answers - Always Visible for First Question */}
        <>
          {/* Peek Header */}
          ...
          {/* Encourage Answer Writing (After viewing others' answers) */}
          {isFreeQuestion && !showMyAnswer && (
          ...
          {/* My Answer Section (When clicked) */}
          {showMyAnswer && !shared && (
