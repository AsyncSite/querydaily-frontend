...
> ⚠️ **최신 업데이트 (2025년 11월 9일)**: 새로운 이벤트 추가 및 시각화 중심 가이드 추가
>
> 📊 **추천:** 비기술 팀원은 [시각화 가이드](./VISUALIZATION-GUIDE.md)를 먼저 확인하세요!
>
> 🔧 **이 문서:** 기술적 상세 설정 (개발자/관리자용)
...
- 확장된 `click_cta`: 네비게이션, FAQ, 슬라이더, 탭 전환

**새로 추가된 파라미터:**
- `engagement_time_seconds`: 페이지 체류 시간
- `exit_scroll_depth`: 이탈 시 스크롤 위치
- `max_scroll_depth`: 최대 스크롤 도달 깊이
- `step_number`: 폼 단계 번호
- `cta_text`: CTA 버튼 텍스트
- `section_name`: 섹션 이름 (명시화 완료)

**커스텀 차원/측정항목 개수 변경:**
- 기존: 차원 4개 + 보너스 3개
- 현재: **차원 7개 필수** + 측정항목 6개 필수
