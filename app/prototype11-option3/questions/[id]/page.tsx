...
export default function QuestionPage({ params }: { params: { id: string } }) {
  ...
  const handleNext = () => {
    if (nextId) {
      router.push(`/prototype11-option3/questions/${nextId}`);
    } else {
      router.push('/prototype11-option3/dashboard');
    }
  };

  const handlePrev = () => {
    if (prevId) {
      router.push(`/prototype11-option3/questions/${prevId}`);
    }
  };
  ...
  return (
    <div className="px-6 py-8 space-y-6">
        ...
        <div className="flex items-center justify-between">
          <Link href="/prototype11-option3/dashboard" className="text-gray-500">
            ← 뒤로
          </Link>
